...
test-components: correlation-engine-test
	...
	@vagrant ssh -c "cd $(RAG_INGESTER_BUILD_DIR) && ctest --output-on-failure"
	@echo ""
	@echo "Testing etcd-server..."
	@vagrant ssh -c "cd $(ETCD_SERVER_BUILD_DIR) && ctest --output-on-failure"
	@echo ""
	@echo "Testing RAG Security..."
	@vagrant ssh -c "cd $(RAG_BUILD_DIR) && ctest --output-on-failure"
	@echo ""
	@echo "Testing Firewall ACL Agent..."
	@vagrant ssh -c "cd $(FIREWALL_BUILD_DIR) && ctest --output-on-failure"
	@echo ""
...
experiment-zeek-replay-10:
	...
	@cd $(ZEEK_DIR) && vagrant ssh zeek -c " \
	  ...
	  cd $(ZEEK_LOGS)/10mbps && \
	  sudo /opt/zeek/bin/zeek -r /vagrant/datasets/ctu13/botnet-capture-20110810-neris.pcap local > zeek-stdout.log 2>&1 && \
	  sleep 5 && \

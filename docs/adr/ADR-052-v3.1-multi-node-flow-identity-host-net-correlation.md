...
|---|---|
| **ADR** | 052 |
| **Versión** | **v3.1 (candidata a ratificación)** — incorpora la 2ª pasada del Consejo (DAY 173), el arbitraje de Alonso y la auto-revisión de v3 (4 correcciones) |
| **Fecha** | DAY 173 (rev. v3.1) |
...
- **Test de paridad cross-implementación obligatorio** (EMECAS++): C++ y Python producen `flow_uid` idéntico sobre el mismo vector **Y enlazan la misma versión de libsodium** (el test verifica ambas cosas). El `seq_in_window` es **input del vector de paridad** (Python lo recibe del evento, no lo computa — §3.1.4), para que el test no asuma que ambas implementaciones lo derivan. Mismo patrón que la paridad `pycommunityid`.

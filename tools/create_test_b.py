#!/usr/bin/env python3
# create_test_b.py — crea test_autonomy_integration.cpp en la VM
from pathlib import Path

OK = '✅'; ERR = '❌'

TEST_B_PATH = Path('/vagrant/etcd-server/tests/test_autonomy_integration.cpp')
TEST_B_PATH.parent.mkdir(parents=True, exist_ok=True)

content = '''// test_autonomy_integration.cpp — DAY 156
...
TEST_F(AutonomyIntegrationTest, InitialStateNoEvent) {
    ml_defender::common::AutonomyPublisher pub(TEST_ENDPOINT, "test", 0);
    ml_defender::CryptoAutonomy sm("test", pub.make_callback());
...
TEST_F(AutonomyIntegrationTest, VaultKoPublishesAutonomous) {
    ml_defender::common::AutonomyPublisher pub(TEST_ENDPOINT, "etcd-server", 0);
...
TEST_B_PATH.write_text(content)
print(f'{OK} {TEST_B_PATH} creado ({len(content)} bytes)')
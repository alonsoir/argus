...
Adapter del contrato bronce `correlation_v1` para **zeek**.
...
zeek_adapter config/zeek_adapter.json [entrada.json]
```

Escribe `<base_dir>/zeek-%Y-%m-%d-%H%M%S.csv` de forma atómica (`.tmp` → rename).
...
- Guard **D-D** diferido: cuando se active, `"zeek"` tendrá que ser un símbolo
...
```sh
python3 tools/scaffold_adapter.py --sensor zeek
```

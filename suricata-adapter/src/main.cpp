// suricata-adapter/src/main.cpp
// aRGus NDR — adapter de suricata, modo LOTE.
//
...
#include "suricata_adapter/config.hpp"
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        std::cerr << "uso: suricata_adapter <config.json> [entrada.json]\n";
        return 2;
    }

    suricata_adapter::Config cfg;
    std::string error;
    if (!suricata_adapter::load_config(argv[1], cfg, error)) {
    ...
    suricata_adapter::BatchWriter writer(cfg.base_dir, suricata_adapter::CORRELATION_SOURCE_SENSOR);
    if (!writer.open()) {
    ...
    while (std::getline(in, line)) {
        ++total;
        auto tr = suricata_adapter::to_row(line, cfg.node_id);

        if (tr.status == suricata_adapter::ToRowResult::Status::Skip) {
            ++skipped;
            continue;
        }
        if (tr.status == suricata_adapter::ToRowResult::Status::Error) {
            ++to_row_err;
    ...
    std::cout << "[" << suricata_adapter::CORRELATION_SOURCE_SENSOR << "-adapter]"
              << " leidas="        << total
              << " escritas="      << written
              << " descartadas="   << skipped
